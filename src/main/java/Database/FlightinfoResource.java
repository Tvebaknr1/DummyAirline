...
public class FlightinfoResource
{
    ...
    @GET
    @Path("{from}/{too}/{date}/{tickets}")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.TEXT_PLAIN)
    public String getFlightsWithFromTooDateTickets(@PathParam("from") String from,@PathParam("too") String too, @PathParam("date") String date, @PathParam("tickets") int tickets) throws RuntimeException, FlightException, NotFoundException
    {
        String[] strArr = date.split("T");
        strArr = strArr[0].split("-");
        Date dateDate = new Date ((Integer.parseInt(strArr[0])-1900),(Integer.parseInt(strArr[1])-1),Integer.parseInt(strArr[2]));
        System.out.println(date.toString()); 
         List<FlightInstance> flights = flightFacade.getFlightsWithFromTooDateTickets(from, too, dateDate, tickets);
        if (flights == null || flights.size() == 0)
        {
            throw new FlightException();
        }
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println(new FlightinfoResource().getFlightsWithFromTooDateTickets("CPH","JFK", "2017-01-08T00:00:00.000Z", 1));
